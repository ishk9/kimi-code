...
When resuming a session, you can override its saved permission or plan mode by adding `--auto`, `--yolo`, or `--plan`. For example, `rain --continue --auto` resumes the latest session and switches it to auto permission mode.
...
- **`--skills-dir <dir>`** (CLI flag): **Replaces** the automatically discovered user and project directories for this launch only. Can be repeated to stack multiple directories:
  ...
  ```sh
  rain --skills-dir /path/to/team-skills --skills-dir ./local-skills
  ```
...
```sh
rain -p "Summarize the current repository status"
```
...
`rain` provides the following subcommands: `login` (non-interactive login), `acp` (ACP IDE mode), `doctor` (validate configuration files), `export` (export a session), `migrate` (migrate legacy data), `upgrade` (check for updates), and `provider` (manage providers).
...
Switch Kimi Code CLI to ACP (Agent Client Protocol) mode, communicating with an IDE via JSON-RPC over stdin/stdout so the editor can directly drive the CLI's sessions and tool calls. You typically do not need to run this manually — the IDE starts it as a subprocess entry point. For configuration, see [Using in IDEs](../guides/ides.md); for technical details, see the [rain acp reference](./kimi-acp.md).
...
| Command | Description |
| --- | --- |
| `rain doctor` | Validate the default `config.toml` and `tui.toml` |
| `rain doctor config [path]` | Validate only `config.toml`, using `path` instead of the default file when provided |
| `rain doctor tui [path]` | Validate only `tui.toml`, using `path` instead of the default file when provided |
...
rain export 01HZ...XYZ -o ./bug-report.zip

# Exclude the global diagnostic log
rain export 01HZ...XYZ -o ./bug-report.zip --no-include-global-log
```
...
For global npm, pnpm, yarn, bun, and macOS / Linux native installations, `rain upgrade` shows update options; selecting `Install update now` runs the corresponding foreground install command. When the current installation method cannot be upgraded automatically (e.g., Windows native installation), the manual update command is printed instead.
...
```sh
rain provider <action> [options]
```

Five actions are available:

#### `rain provider add <url>`
...
```sh
rain provider add https://registry.example.com/v1/models/api.json --api-key YOUR_KEY

# Or via environment variable (suitable for CI / .envrc)
KIMI_REGISTRY_API_KEY=YOUR_KEY rain provider add https://registry.example.com/v1/models/api.json
```
...
#### `rain provider remove <providerId>`
...
```sh
rain provider list
rain provider list --json | jq '.providers | keys'
```

#### `rain provider catalog list [providerId]`
...
```sh
rain provider catalog list
rain provider catalog list --filter anthropic
rain provider catalog list anthropic
```

#### `rain provider catalog add <providerId>`
...
```sh
rain provider catalog list anthropic          # Browse available models first
rain provider catalog add anthropic --api-key sk-ant-... --default-model claude-opus-4-7
```
